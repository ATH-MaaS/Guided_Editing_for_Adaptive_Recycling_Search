...
from VideoReward.vision_process import process_vision_info
from VideoReward.data import DataConfig
from VideoReward.utils import ModelConfig, PEFTLoraConfig, TrainingConfig
from VideoReward.utils import load_model_from_checkpoint
from VideoReward.inference_model import create_model_and_processor
from VideoReward.prompt_template import build_prompt
...
class VideoVLMRewardInference():
    ...
    def prepare_batch(self, video_paths, prompts, fps=None, num_frames=None, max_pixels=None,):
        ...
        return batch
    def prepare_infer_batch(self, videos, prompts, fps=None, num_frames=None, max_pixels=None,):
        fps = self.data_config.fps if fps is None else fps
        num_frames = self.data_config.num_frames if num_frames is None else num_frames
        max_pixels = self.data_config.max_frame_pixels if max_pixels is None else max_pixels
        ...
        # Normalize pixel values to [0, 1] to match training preprocessing.
        # When input is a file path, fetch_video returns [0, 255] range;
        # when input is a tensor (e.g. from EvoSearch), fetch_video already
        # maps [-1, 1] -> [0, 1]. With do_rescale=False the processor skips
        # its own rescaling, so we must ensure [0, 1] here.
        video_inputs = [v.float() / 255.0 if v.max() > 1.0 else v for v in video_inputs]
    ...
    def reward(self, video_paths, prompts, fps=None, num_frames=None, max_pixels=None, use_norm=True):
        ...
        batch = self.prepare_infer_batch(video_paths, prompts, fps, num_frames, max_pixels)
        rewards = self.model(
            return_dict=True,
            **batch
        )["logits"]
        ...
        return rewards
    # def score(video,prompt,device,dtype):
    #     load_from_pretrained = "./checkpoints"
        
    #     rewards = inferencer.reward(video, prompt, use_norm=True)
    #     return rewards
