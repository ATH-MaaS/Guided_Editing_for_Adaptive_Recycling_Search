...
class FlowDPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def dpm_solver_first_order_update(
        ...
        **kwargs,
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            variance = sigma_t * torch.sqrt(1.0 - torch.exp(-2 * h)) * noise
            std = sigma_t * torch.sqrt(1.0 - torch.exp(-2 * h))
        elif self.config.algorithm_type == "sde-dpmsolver":
            assert noise is not None
            x_t = ((alpha_t / alpha_s) * sample - 2.0 *
                   (sigma_t * (torch.exp(h) - 1.0)) * model_output +
                   sigma_t * torch.sqrt(torch.exp(2 * h) - 1.0) * noise)
        return x_t, variance, std  # pyright: ignore
    ...
    def step(
        ...
        if self.config.solver_order == 1 or self.lower_order_nums < 1 or lower_order_final:
            prev_sample, variance, std = self.dpm_solver_first_order_update(
                model_output, sample=sample, noise=noise)
        elif self.config.solver_order == 2 or self.lower_order_nums < 2 or lower_order_second:
            prev_sample, variance, std = self.multistep_dpm_solver_second_order_update(
        ...
        if not return_dict:
            return (prev_sample, model_output, variance, std)
